    ...
    post {
        ...
        success {
            withCredentials([string(credentialsId: 'Discord_Webhook', variable: 'WEBHOOK_URL')]) {
                sh '''
                    curl -H 'Content-Type: application/json' -d '{\"content\": \"✅ Micropython v'"$VERSION"' Released to AUR 🎆\"}' ${WEBHOOK_URL}
                '''
            }
        }
        failure {
            withCredentials([string(credentialsId: 'Discord_Webhook', variable: 'WEBHOOK_URL')]) {
                sh '''
                    curl -H 'Content-Type: application/json' -d '{\"content\": \"❌ Micropython v'"$VERSION"' Pipeline Failed 🛠️\"}' ${WEBHOOK_URL}
                '''
            }
        }
    }
